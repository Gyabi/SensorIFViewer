...
class PlotData {
  ...
  PlotData({required this.id, required this.latitude, required this.longitude, required this.lastUpdated});
}
...
class MarkerBloc extends Bloc<MarkerEvent, MarkerState> {
  ...
  MarkerBloc({required this.udpReceiver}): super(MarkerState([])) {
    ...
    _cleanupTimer = Timer.periodic(Duration(milliseconds: 100), (_) {
      ...
      plotDataMap.removeWhere((_, data) => now.difference(data.lastUpdated).inMilliseconds > 200);
