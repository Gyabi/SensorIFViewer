...
class CoreState {
  ...
  CoreState({required this.status});

  CoreState copyWith({CoreStatus? status, List<LatLng>? positions}) {
    return CoreState(status: status ?? this.status);
  }
}
...
class CoreBloc extends Bloc<CoreEvent, CoreState> {
  ...
  CoreBloc({required this.repository, required this.udpReceiver})
    : super(CoreState(status: CoreStatus.ready)) {
